  ...
  var AbstractWebClient = Widget.extend(KeyboardNavigationMixin, {
    ...
    custom_events: {
      ...
      do_action: function (event) {
        const actionProm = this.do_action(
          event.data.action,
          event.data.options || {}
        );
        this.menu_dp
    ...
    start: function () {
      ...
      if (!state.cids) {
        state.cids =
          utils.get_cookie("cids") !== null
            ? utils.get_cookie("cids")
      ...
      $("link[type='image/x-icon']").attr(
        "href",
        "/web/image/res.company/" + String(stateCompanyIDS[0]) + "/favicon/"
      );
    ...
    bind_events: function () {
      ...
      this.$el.on(
        "mouseenter",
        ".oe_systray > div:not([data-toggle=tooltip])",
        function () {
          $(this)
      ...
      core.bus.on("click", this, function (ev) {
        ...
        if (!$(ev.target).is("input[type=file]")) {
          $(
            this.el.getElementsByClassName("oe_dropdown_menu oe_opened")
          ).removeClass("oe_opened");
          $(
            this.el.getElementsByClassName("oe_dropdown_toggle oe_opened")
          ).removeClass("oe_opened");
        }
      });
    ...
    do_reload: function () {
      ...
      return session.session_reload().then(function () {
        session.load_modules(true).then(self.menu.proxy("do_reload"));
      });
    },
    do_push_state: function (state) {
      if (!state.menu_id && this.menu) {
        // this.menu doesn't exist in the POS
    ...
    getScrollPosition: function () {
      var scrollingEl =
        this.action_manager.el.getElementsByClassName("o_content")[0];
      return {
    ...
    _onCreateFilter: function (e) {
      data_manager.create_filter(e.data.filter).then(e.data.on_success);
    },
    /**
     * @private
     * @param {OdooEvent} e
    ...
    _onDeleteFilter: function (e) {
      data_manager.delete_filter(e.data.filterId).then(e.data.on_success);
    },
    /**
    ...
    _onPushState: function (e) {
      this.do_push_state(
        _.extend(e.data.state, { cids: $.bbq.getState().cids })
      );
    },
    /**
    ...
    _onWebClientStarted: function () {
      if (!this.isStarted) {
        ...
        this.action_manager.$el.on(
          "scroll",
          core.bus.trigger.bind(core.bus, "scroll")
        );
        odoo.isReady = true;
